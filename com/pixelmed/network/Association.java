...
public class Association {
	...
	private static final String identString = "@(#) $Header: /userland/cvs/pixelmed/imgbook/com/pixelmed/network/Association.java,v 1.52 2011/11/01 17:36:10 dclunie Exp $";
...
if (debugLevel > 1) System.err.println("Association["+associationNumber+"].setSocketOptions(): getReceiveBufferSize() = "+socket.getReceiveBufferSize());
if (debugLevel > 1) System.err.println("Association["+associationNumber+"].setSocketOptions(): getSendBufferSize() = "+socket.getSendBufferSize());
if (debugLevel > 1) System.err.println("Association["+associationNumber+"].setSocketOptions(): getSoLinger() = "+socket.getSoLinger());
if (debugLevel > 1) System.err.println("Association["+associationNumber+"].setSocketOptions(): getSoTimeout() = "+socket.getSoTimeout());
if (debugLevel > 1) System.err.println("Association["+associationNumber+"].setSocketOptions(): getTcpNoDelay() = "+socket.getTcpNoDelay());
...
if (debugLevel > 1) System.err.println("Association["+associationNumber+"].setSocketOptions(): asking to change receiveBufferSize to = "+socketReceiveBufferSize);
			socket.setReceiveBufferSize(socketReceiveBufferSize);
if (debugLevel > 1) System.err.println("Association["+associationNumber+"].setSocketOptions(): receiveBufferSize changed to = "+socket.getReceiveBufferSize());
		}
			
		if (socketSendBufferSize != 0 && socket.getSendBufferSize() != socketSendBufferSize) {
if (debugLevel > 1) System.err.println("Association["+associationNumber+"].setSocketOptions(): asking to change sendBufferSize to = "+socketSendBufferSize);
			socket.setSendBufferSize(socketSendBufferSize);
if (debugLevel > 1) System.err.println("Association["+associationNumber+"].setSocketOptions(): sendBufferSize changed to = "+socket.getSendBufferSize());
		}
...
//System.err.println("Association["+associationNumber+"].getRestOfPDU(): startBuffer.length="+startBuffer.length+" pduLength="+pduLength);
	...
	public void release() throws DicomNetworkException {
		...
		try {
if (debugLevel > 1) System.err.println("Association["+associationNumber+"]: Us: A-RELEASE-RQ");
...
if (debugLevel > 1) System.err.println("Association["+associationNumber+"]: Us: close transport connection");
				socket.close();							// AR-3      - Issue A-RELEASE confirmation primitive and close transport connection
if (debugLevel > 1) System.err.println("Association["+associationNumber+"]: Us: Transport connection is "+(socket.isClosed() ? "" : "not ")+"closed after close() request");
...
if (debugLevel > 1) System.err.println("Association["+associationNumber+"]: Them: A-RELEASE-RQ (collision)\n");
												// AR-8      - we were the association requester, so go to State 9
												// State 9   - 
if (debugLevel > 1) System.err.println("Association["+associationNumber+"]: Us: A-RELEASE-RP");
...
if (debugLevel > 1) System.err.println("Association["+associationNumber+"]: Us: A-ABORT");
...
if (debugLevel > 1) System.err.println("Association["+associationNumber+"].send(): Us: P-DATA-TF=\n"+pdu);
...
if (debugLevel > 1) System.err.println("Association["+associationNumber+"]: Waiting to close transport connection.");
		for (int time=0; time < associationReleaseToTransportConnectionCloseTimeoutInMilliseconds; time+=associationReleaseToTransportConnectionCloseCheckAlreadyClosedIntervalInMilliseconds) {
			if (!socket.isClosed()) {
if (debugLevel > 1) System.err.println("Association["+associationNumber+"]: Other end closed transport connection during ARTIM.");
				break;
			}
			wait(associationReleaseToTransportConnectionCloseCheckAlreadyClosedIntervalInMilliseconds);
		}
if (debugLevel > 1) System.err.println("Association["+associationNumber+"]: Closing transport connection.");
	}
...
if (debugLevel > 1) System.err.println("Association["+associationNumber+"]: Them: PDU Type: 0x"+Integer.toHexString(pduType)+" (length "+pduLength+" dec 0x"+Integer.toHexString(pduLength)+")");

			if (pduType == 0x04) {							//           - P-DATA PDU
				PDataPDU pdata = new PDataPDU(getRestOfPDU(in,startBuffer,pduLength));
if (debugLevel > 2) System.err.println("Association["+associationNumber+"]: Them:\n"+pdata);
				receivedDataHandler.sendPDataIndication(pdata,this);		// DT-2      - send P-DATA indication primitive
												// State 6   - Data Transfer
if (debugLevel > 2) System.err.println("Association["+associationNumber+"]: stopAfterLastFragmentOfCommand="+stopAfterLastFragmentOfCommand);
if (debugLevel > 2) System.err.println("Association["+associationNumber+"]: pdata.containsLastCommandFragment()="+pdata.containsLastCommandFragment());
if (debugLevel > 2) System.err.println("Association["+associationNumber+"]: stopAfterLastFragmentOfData="+stopAfterLastFragmentOfData);
if (debugLevel > 2) System.err.println("Association["+associationNumber+"]: pdata.containsLastDataFragment()="+pdata.containsLastDataFragment());
if (debugLevel > 2) System.err.println("Association["+associationNumber+"]: stopAfterHandlerReportsDone="+stopAfterHandlerReportsDone);
if (debugLevel > 2) System.err.println("Association["+associationNumber+"]: receivedDataHandler.isDone()="+receivedDataHandler.isDone());
...
if (debugLevel > 2) System.err.println("Association["+associationNumber+"]: waitForPDataPDUs is stopping");
					break;
				}
			}
			else if (pduType == 0x05) {						//           - A-RELEASE-RQ PDU
if (debugLevel > 1) System.err.println("Association["+associationNumber+"]: Them: A-RELEASE-RQ");
...
if (debugLevel > 1) System.err.println("Association["+associationNumber+"]: Us: A-RELEASE-RP");
				...
				out.write(arr.getBytes());
				out.flush();
if (debugLevel > 1) System.err.println("Association["+associationNumber+"]: Awaiting Transport connection close");
												// State 13  - Awaiting Transport connection close
