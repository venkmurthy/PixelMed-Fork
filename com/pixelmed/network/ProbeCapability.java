...
public class ProbeCapability implements Runnable {
	...
	/***/
	private static final String identString = "@(#) $Header: /userland/cvs/pixelmed/imgbook/com/pixelmed/network/ProbeCapability.java,v 1.9 2012/02/01 23:02:12 dclunie Exp $";
...
if (debugLevel > 1) System.err.println("Storage test association "+association);
...
if (debugLevel > 0) System.err.println("Supported Storage SOP Classes "+supportedStorageSOPClasses);
		}
		catch (Exception e) {
if (debugLevel > 1) e.printStackTrace(System.err);
		...
		try {
			supportedQuerySOPClasses = probeSupportedSOPClasses(
				...
				hostname,port,calledAETitle,callingAETitle,debugLevel);
if (debugLevel > 0) System.err.println("Supported Query SOP Classes "+supportedQuerySOPClasses);
...
if (debugLevel > 1) e.printStackTrace(System.err);
		...
		try {
			supportedRetrieveWithMoveSOPClasses = probeSupportedSOPClasses(
				...
				hostname,port,calledAETitle,callingAETitle,debugLevel);
if (debugLevel > 0) System.err.println("Supported Retrieve with Move SOP Classes "+supportedRetrieveWithMoveSOPClasses);
...
if (debugLevel > 1) e.printStackTrace(System.err);
		...
		try {
			supportedRetrieveWithGetSOPClasses = probeSupportedSOPClasses(
				...
				hostname,port,calledAETitle,callingAETitle,debugLevel);
if (debugLevel > 0) System.err.println("Supported Retrieve with Get SOP Classes "+supportedRetrieveWithGetSOPClasses);
...
if (debugLevel > 1) System.err.println("Trying port "+p);
					try {
...
e.printStackTrace(System.err);
				...
				if (port != 0) {
if (debugLevel > 0) System.err.println("ProbeCapability: successful verification on port "+port);
				}
			}
	...
	//static boolean canConnectToPort(InetAddress address,int p) {
	static boolean canConnectToPort(String hostAddress,int p) {
		boolean success = false;
		//String hostAddress = address.getHostAddress();		// should just return IP address without blocking for reverse lookup
System.err.println("Trying to connect to host "+hostAddress+" on port "+p);
		try {
			Socket socket = new Socket(hostAddress,p);
			//new VerificationSOPClassSCU(hostAddress,p,"THEM","US",2);
